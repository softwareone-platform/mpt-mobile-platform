...
class SpotlightsPage extends BasePage {
  ...
  get filterScrollView() {
    return $(
      getSelector({
        ...
        android:
          '//android.widget.HorizontalScrollView[.//*[contains(@resource-id, "spotlight-filter-")]]',
      }),
    );
  }
  ...
  async horizontalSwipeOnFilters(direction) {
    if (isAndroid()) {
      ...
      // iOS: Get filter scroll view location and perform coordinate-based swipe
      const filterContainer = await this.filterScrollView;
      const location = await filterContainer.getLocation();
      const size = await filterContainer.getSize();

      // Calculate swipe coordinates within the filter container
  ...
  async selectFilter(filterName) {
    ...
    const selector = isAndroid() ? `//*[@resource-id="${resourceId}"]` : `~${resourceId}`;
   ...
   * Note: The filter component doesn't expose accessibility state, so selection
   * state cannot be reliably determined via automation attributes.
   * Section visibility tests should be used to validate actual filter behavior.
   * This method returns false to indicate selection state cannot be determined.
   * @param {string} filterName - filter name
   */
  async isFilterSelected(filterName) {
    // The FiltersHorizontal component uses visual styling (chipActive) to indicate
    // selection but doesn't expose this via accessibility attributes.
    // Return false to indicate we cannot determine selection state.
    // Tests should rely on section visibility to validate filter behavior.
    return false;
  }
