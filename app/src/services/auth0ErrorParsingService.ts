...
class Auth0ErrorParsingService {
  ...
  private extractErrorDetails(error: Error): Auth0ErrorDetails {
    const errorAny = error as ErrorWithDetails;

    const name =
      error.name !== 'Error' && error.name !== 'unknown_error'
        ? error.name
  ...
  private isEmailNotAuthorizedError(
    errorName: string,
    errorMessage: string,
    status?: number,
  ): boolean {
    return (
      errorName === 'access_denied' || (errorMessage.includes('linking account') && status === 500)
    );
  }
